...
$result3 = mysqli_query($con1, "Select * from tblexpense1 where store='$store'");
if (mysqli_num_rows($result3)>0){?>
    <table class="styled-table" id="table">
        ...
        while ($res=mysqli_fetch_array($result3)) {
            ...
            $result4 = mysqli_query($con1, "Select * from tblexpensecategory where category_id='$category3'");
            ...
            returnCategory(this.cells[2].innerHTML);
            returnAmount(this.cells[0].innerHTML);
            returnTable(this.cells[0].innerHTML)">
                <?php
                echo '<td>'.$res['expense_id'].'</td>';
                echo '<td>'.$res['expense_date'].'</td>';
                echo '<td>'.$category.'</td>';?>
            </tr>
            <?php
        }
